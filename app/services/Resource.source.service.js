(function () {
  ...
  function ResourceSource($http, Global, Util, ErrorHandler, QueryFactory) {
    ...
    function getReferenceSource(name) {
      ...
      sparql +=
        "OPTIONAL { ?source " + Global.ns + "description ?description. }. ";
      ...
      QueryFactory.ExecuteQuery(sparql).then(function (result) {
        ...
        if (!result.error) {
          ...
          if (result.language) factory.language = result.language.value;
          if (result.year) factory.year = result.year.value;
          if (result.url) factory.url = result.url.value;
        }
      });
    }
