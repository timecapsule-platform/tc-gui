(function () {
  ...
  function Data(
    ...
    function Get(resource, searchTerm) {
      ...
      } else if (resource == "Drug Component") {
        return GetDrugs(searchTerm);
      } else if (resource == "Reference Source") {
        return GetSources(searchTerm);
      } else if (resource == "Location") {
        return GetLocations(searchTerm);
      } else if (resource == "Plant Part") {
        return GetPlantParts(searchTerm);
      } else if (resource == "Plant Family") {
        return GetFamilies(searchTerm);
      } else if (resource == "Publication Location") {
    ...
    function GetPublicationLocations(searchTerm) {
      return DataFactory.GetPublicationLocations(searchTerm).then(function (
        result
      ) {
        if (!result.error) {
