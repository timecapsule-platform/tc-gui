...
function Company($http, $state, Global, Util) {
    ...
    {   
        return $http.get(api + '/subscribers/'+subscriberId+'/companies').then(handleSuccess, handleError);
    }
    
    // Get the current company from the SESSION
    function getCurrentCompany()
    {   
        return $http.get(api + '/session/company').then(handleSuccess, handleError);
    }
    
    // Sets the current company to the SESSION
    function setCurrentCompany(index)
    {   
        return $http.put(api + '/session/company',index).then(handleSuccess, handleError);
    }
