(function () {
  ...
  function ResourceDrug($http, Global, Util, ErrorHandler, QueryFactory) {
    ...
    function getReferences(concept) {
      ...
      sparql +=
        "SELECT DISTINCT ?name ?drug ?lineNumber ?barcode ?id  ?plantName ?plantVariant ?locationName ?latitude ?longtitude ?referenceSourceName ?referenceSourceDate ?referenceSourceLanguage ";
      sparql += "WHERE { ";
      ...
      sparql += "?nv " + Global.ns + "nameVariant ?name. ";
      sparql +=
        "?nv " + Global.ns + 'nameVariant "' + concept + '"^^xsd:string. ';
      sparql += "?reference " + Global.ns + "refersTo ?nv. ";
