(function(){
    ...
    angular.module("TIMECAPSULE").directive("piePlantFamily", piePlantFamilyDirective);
    ...
    function piePlantFamilyDirective()
    {
        ...
        return {
                restrict: "E",
                templateUrl: "app/directives/pie-plant-family/pie-plant-family.template.html",
                scope: 
                { 
                    title : "@",
                    family: "@"
     ...
     function piePlantFamilyController($scope,Util,Global,PopupService,QueryFactory)
    {
         ...
         $scope.buildVisualizationData = function()
        {
             ...
             sparql += '?family  ' + Global.ns + 'name "'+$scope.family+'"^^xsd:string. ';
            ...
            QueryFactory.ExecuteQuery(sparql).then(function(result)
             {   
                ...
                {   
                    ...
                    {   
                        if(!Util.exists($scope.concepts,$scope.references[i].plantName.value))
                        {
        ...
        {   
            ...
            for(var i=0;i<$scope.occurences.length;i++)
            {
                $scope.percentages[i] =  Util.roundNum($scope.occurences[i] / total * 100,1); 
            }
