...
function progressBarLink($scope, element, attrs) 
{
    ...
    {  
      ...
      if ($scope.maxVal) 
      {
        progress = Math.min($scope.curVal, $scope.maxVal) / $scope.maxVal * element[0].querySelector('.progress-bar').clientWidth;
      }

      element[0].querySelector('.progress-bar-bar').style.width = progress +"px";    
