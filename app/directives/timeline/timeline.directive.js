  ...
  function timelineController($scope, Util, PopupService) {
    ...
    $scope.initTimeline = function () {
      for (var i = 0; i < $scope.references.length; i++) {
        ...
        if (
          $scope.type == "Variants" ||
          $scope.type == "Language" ||
          $scope.type == "Source Language" ||
          $scope.type == "Source Location"
        ) {
        ...
        if (date) {
          ...
          } else if (date > $scope.date.to) {
            $scope.date.to = date;
          }
        }
      }

      $scope.graph.setWindow(
        $scope.date.from + "-01-01",
        $scope.date.to + "-01-01"
      );
    };
    ...
    $scope.generateConcepts = function () {
      for (var i = 0; i < $scope.references.length; i++) {
        ...
        // Language in time
        else if ($scope.type == "Language" && $scope.references[i].language) {
        ...
        // Source Language in time
          ...
          $scope.type == "Source Language" &&
          $scope.references[i].sourceLanguage
        ) {
        ...
        // Location in time
          ...
          $scope.type == "Source Location" &&
          $scope.references[i].sourceLocation
        ) {
    ...
    $scope.insertGraphData = function (newItem) {
      if (!$scope.graphData.length) {
        ...
        for (var i = 0; i < $scope.graphData.length; i++) {
          if (
            $scope.graphData[i].group == newItem.group &&
            $scope.graphData[i].x == newItem.x
          ) {
            var isNew = false;
        ...
        for (var j = 0; j < $scope.references.length; j++) {
          ...
          if (date) {
            ...
            if ($scope.type == "Variants" && $scope.references[j].nameVariant) {
              ...
              $scope.type == "Language" &&
              $scope.references[j].language
            ) {
              concept = $scope.references[j].language.value;
            } else if (
              $scope.type == "Source Language" &&
              $scope.references[j].sourceLanguage
            ) {
              concept = $scope.references[j].sourceLanguage.value;
            } else if (
              $scope.type == "Source Location" &&
              $scope.references[j].sourceLocation
            ) {
