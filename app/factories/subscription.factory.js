(function () {
    ...
    angular.module('TIMECAPSULE').factory('SubscriptionFactory', SubscriptionFactory);
...
function SubscriptionFactory($http, Global) {
    ...
    {   
        return $http.get(api + '/subscribers/'+id).then(handleSuccess, handleError);
    }
     
    // Get the subscriber's subscriptions
    function GetSubscription(id)
    {   
        return $http.get(api + '/subscribers/'+id+'/subscription').then(handleSuccess, handleError);
    }
