(function(){
	...
	cargoController.$inject = ["$scope","$state","Global","Auth","Navigation","Util","Data","PopupService","MessageService", "ErrorHandler", "DataFactory","QueryFactory","NgMap"];
	
	
	
	// Controller Logic
	function cargoController($scope,$state,Global,Auth,Navigation,Util,Data,PopupService,MessageService, ErrorHandler, DataFactory, QueryFactory,NgMap)
	{ 
         // Urls for the View
        ...
        $scope.$watch(function () { return $scope.filters.ship.search },function()
        {
            $scope.getShips();
        });
        
         $scope.$watch(function () { return $scope.filters.arrival.search },function()
        {
            $scope.getArrivals();
        });
        
        $scope.$watch(function () { return $scope.filters.departure.search },function()
        {
            $scope.getDepartures();
        });
        
        $scope.$watch(function () { return $scope.filters.ship.selected },function()
        {
        ...
        $scope.$watch(function () { return $scope.filters.departure.selected },function()
        {
            if($scope.item && $scope.item.name)
            {
        ...
        $scope.$watch(function () { return $scope.filters.arrival.selected },function()
        {
            if($scope.item && $scope.item.name)
            {
                $scope.countResults($scope.item.name);
        ...
        $scope.$watch(function () { return $scope.filters.date },function()
        {
            if($scope.item && $scope.item.name)
            {
                $scope.countResults($scope.item.name);
        ...
        $scope.showMap = function()
        {
            ...
            setTimeout(function() { $scope.generateMarkers() }, 1000);
        ...
        $scope.generateMarkers = function()
        {
            NgMap.getMap().then(function(map) { 
             ...
             { 
                //icon: markerIcon,
                position: { lat: Number($scope.item.alatitude.value), lng: Number($scope.item.alongitude.value)},
                map: map
             });
            ...
            { 
                //icon: markerIcon,
                position: { lat: Number($scope.item.dlatitude.value), lng: Number($scope.item.dlongitude.value)},
                map: map
             });
             ...
             $scope.route = 
             [
                 { lat: Number($scope.item.dlatitude.value), lng: Number($scope.item.dlongitude.value) },
                 { lat: Number($scope.item.alatitude.value), lng: Number($scope.item.alongitude.value) }
             ];
            ...
            $scope.polyline.setMap(map);
             ...
             if($scope.item.alatitude && $scope.item.alongitude)
             {
                 ...
                 aInfoText += "<b>Latitude: "+$scope.item.alatitude.value+"</b><br/>";
                 aInfoText += "<b>Longitude: "+$scope.item.alongitude.value+"</b><br/>";
                 // Add the infowindow
             ...
             if($scope.item.dlatitude && $scope.item.dlongitude)
             {
                 ...
                 dInfoText += "<b>Latitude: "+$scope.item.dlatitude.value+"</b><br/>";
                 dInfoText += "<b>Longitude: "+$scope.item.dlongitude.value+"</b><br/>";
                 // Add the infowindow
        ...
        {
            return ( $scope.cargoFilter.type == "All" || $scope.cargoFilter.type == cargo.type);
        }
        ...
        $scope.setCargoFilter = function(filter)
        {
            if(filter == "All")
            {
               $scope.cargoFilter =  { all: true,  plants: false, animals: false, minerals: false, type: filter }; 
                
            }
            else
            if(filter == "Plant")
            {
               $scope.cargoFilter =  { all: false,  plants: true, animals: false, minerals: false, type: filter  }; 
            }
            else
            if(filter == "Animal")
            {
               $scope.cargoFilter =  { all: false,  plants: false, animals: true, minerals: false, type: filter  }; 
            }
            else
            if(filter == "Mineral")
            {
               $scope.cargoFilter =  { all: false,  plants: false, animals: false, minerals: true, type: filter  }; 
            }
        }
        ...
        /*********************  Popups  *********************************/
         ...
         $scope.showDetailsPopup = function(item)
         {
             ...
             $scope.popup.dialog = Util.showPopup($scope.popupUrl+'details.popup.html',$scope);
             $scope.showMap();
         }
         ...
         $scope.showShipsPopup = function()
         {
             ...
             $scope.popup.dialog = Util.showPopup($scope.popupUrl+'ships.popup.html',$scope);
         }
         ...
         $scope.showArrivalsPopup = function()
         {
             ...
             $scope.popup.dialog  = Util.showPopup($scope.popupUrl+'arrivals.popup.html',$scope);
         }
         ...
         $scope.showDeparturesPopup = function()
         {
             ...
             $scope.popup.dialog  = Util.showPopup($scope.popupUrl+'departures.popup.html',$scope);
         }
         ...
         $scope.showDatePopup = function()
         {
             ...
             $scope.popup.dialog  = Util.showPopup($scope.popupUrl+'date.popup.html',$scope);
         }
        ...
        {   
             ...
             sparql += 'SELECT DISTINCT  ?ship ?departure ?arrival ?dateDeparture ?dateArrival ?category ?description ?alatitude ?alongitude ?dlatitude ?dlongitude ?quantityValue ?quantityUnit ';
             sparql += 'FROM <http://timecapsule.science.uu.nl/TimeCapsule.owl/boekhouder> '
             sparql += 'WHERE { ';
              
             sparql += '?nv ' + Global.ns + 'nameVariant "'+cargo+'"^^xsd:string. ';
             ...
             sparql += ' ?departureLocation ' + Global.ns + 'name ?departure.';
                ...
                {
                 sparql += ' FILTER(xsd:date(?dateDeparture) > "'+$scope.filters.date.from+'-01-01"^^xsd:date). ';
                }
                sparql += ' }. '; 
                sparql += 'OPTIONAL {';
             ...
             sparql += ' ?arrivalLocation ' + Global.ns + 'name ?arrival.';
                ...
                if($scope.filters.date)
                {
                 sparql += ' FILTER(xsd:date(?dateArrival) < "'+$scope.filters.date.to+'-01-01"^^xsd:date). '; 
                }
                sparql += ' }. '; 
                sparql += 'OPTIONAL {';
             ...
             {
                  sparql +='?voyage ' + Global.ns + 'hasShip ?s. '; 
                  sparql +='?s ' + Global.ns + 'name \"'+$scope.filters.ship.selected.name+'\"^^xsd:string. '; 
             }
             ...
             if($scope.filters.departure.selected)
             {
                  sparql += ' ?voyage ' + Global.ns + 'hasDeparture ?d.';
                  sparql += ' ?d ' + Global.ns + 'hasLocation ?departureLocation.';
                  sparql += ' ?departureLocation ' + Global.ns + 'name \"'+$scope.filters.departure.selected.name+'\"^^xsd:string. '; 
             ...
             if($scope.filters.arrival.selected)
             {
                  sparql += ' ?voyage ' + Global.ns + 'hasArrival ?a.';
                  sparql += ' ?a ' + Global.ns + 'hasLocation ?arrivalLocation.';
                  sparql += ' ?arrivalLocation ' + Global.ns + 'name \"'+$scope.filters.arrival.selected.name+'\"^^xsd:string. '; 
        ...
        {   console.log($scope.filters);
             ...
             sparql += ' } ORDER BY ASC(?dateDeparture) LIMIT '+$scope.resultsLimit+' OFFSET 0'; 
        ...
        {   
             ...
             sparql += ' } ORDER BY ASC(?dateDeparture) OFFSET '+$scope.resultsLimit+' LIMIT 1000'; 
		 ...
		 {  
			 DataFactory.GetDepartures($scope.filters.departure.search).then(function(result)
			 {    
                if(!result.error)
                {   
		 ...
		 {  
			 DataFactory.GetArrivals($scope.filters.arrival.search).then(function(result)
			 {    
                if(!result.error)
                {   
