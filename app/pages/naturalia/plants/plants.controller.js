(function(){
	...
	angular.module("TIMECAPSULE").controller('plantsController', plantsController);

	// Inject services to the Controller
	plantsController.$inject = ["$scope","$rootScope","$state","Global","Auth","Navigation","Util","MessageService", "DBpediaFactory", "ErrorHandler","PopupService","PlantsFactory","QueryFactory","Data","Resource","ResourcePlant","NgMap"];
	
	
	
	// Controller Logic
	function plantsController($scope,$rootScope,$state,Global,Auth,Navigation,Util,MessageService, DBpediaFactory, ErrorHandler, PopupService,PlantsFactory,QueryFactory,Data,Resource,ResourcePlant,NgMap)
	{ 
         // Urls for the View
        ...
        $scope.findItemImages = function()
        {
            ...
            {
                if(($scope.item.name.toLowerCase()).indexOf($scope.data.SnipendalImages[i].name) != -1)
                {
        ...
        $scope.showResource = function(type,name,concept)
        {
         ...
         $scope.showEditPopup = function(item)
         {
             ...
             $scope.popup.dialog = Util.showPopup($scope.popupUrl+'edit.popup.html',$scope);
         }
         ...
         $scope.showCreatePopup = function()
         {
             ...
             $scope.popup.dialog = Util.showPopup($scope.popupUrl+'create.popup.html',$scope);
         }
		  
         $scope.showSortFiltersPopup = function()
         {
              $scope.popup.dialog = Util.showPopup($scope.popupUrl+'sort.popup.html',$scope);
         }
        ...
        { 
             ...
             var sparql = "";            
             sparql += 'SELECT DISTINCT ?nameVariant ?lineNumber ?barcode ?id ?sourceName ?point ?locationName ?locationType ?type ?latitude ?longitude ';
             ...
             sparql += 'OPTIONAL{ ?reference ' + Global.ns + 'locationPropertyType ?type. }.';
             ...
             sparql += ' OPTIONAL { ?location ' + Global.ns + 'locationType ?locationType. }. ';
             sparql += ' }.';  
             ...
             {    
                ...
                {   
                    ...
                    { 
                        ...
                        if(references[i].barcode)
                        {
                            id = references[i].barcode.value;
                            idText = "Barcode"
                        }
                        else
                        if(references[i].lineNumber)
                        {
                            id = references[i].lineNumber.value;
                            idText = "Line Number"
                        }  
                        
                        var infoText = " <b>Name Variant:</b> " +  references[i].nameVariant.value + " <br/>";
                            infoText = infoText + "<b>Reference Source:</b> " +  references[i].sourceName.value + " <br/>";
         ...
         $scope.getAllPlantReferences = function(name)
        {
             ...
             sparql += 'SELECT DISTINCT  ?name ?language ?reference  ?lineNumber ?barcode ?id  ?drugName ?drugVariant ?locationName ?latitude ?longtitude ?sourceName';
             sparql += $scope.fromGraphs;
             sparql += 'WHERE { ';
             sparql += '?p  a ex:Plant. ';
             sparql += '?p ' + Global.ns + 'name "'+name+'"^^xsd:string. ';
         ...
         $scope.getReferencesCount = function(name)
        {
             ...
             sparql += 'SELECT DISTINCT  ?name ?language ?reference  ?lineNumber ?barcode ?id  ?drugName ?drugVariant  ?sourceName   ';
             sparql += $scope.fromGraphs;
             sparql += 'WHERE { ';
             sparql += '?p  a ex:Plant. ';
             sparql += '?p ' + Global.ns + 'name "'+name+'"^^xsd:string. ';
         ...
         $scope.getReferences = function(name)
        {
             ...
             sparql += 'SELECT DISTINCT  ?name ?language ?reference  ?lineNumber ?barcode ?id  ?drugName ?drugVariant ?locationName ?latitude ?longtitude ?sourceName ?sourceType ';
             sparql += $scope.fromGraphs;
             sparql += 'WHERE { ';
             sparql += '?p  a ex:Plant. ';
             sparql += '?p ' + Global.ns + 'name "'+name+'"^^xsd:string. ';
