(function(){
	...
	angular.module("TIMECAPSULE").controller('animalsController', animalsController);

	// Inject services to the Controller
	animalsController.$inject = ["$scope","$state","Auth","Navigation","Util","MessageService", "ErrorHandler","PopupService"];
	
	
	
	// Controller Logic
	function animalsController($scope,$state,Auth,Navigation,Util,MessageService, ProductsFactory, ErrorHandler, PopupService)
	{ 
         // Urls for the View
         $scope.url = 'app/pages/naturalia/animals/';
         $scope.popupUrl = 'app/pages/naturalia/animals/popup/'
        ...
        $scope.applyFilter = function(worker)
		 {	 
			 ...
			 if (
                ( $scope.filter === "All") ||
                ( $scope.filter === "Pending" && questionnaire.status === "Running" && Number(questionnaire.submited) === 0 ) ||
                ( $scope.filter === "Completed" && Number(questionnaire.submited) === 1 ) || 
                ( $scope.filter === "Expired" && questionnaire.status === "Expired" && Number(questionnaire.submited) === 0 ) ||
        ...
        /********************* Data Collectors  *********************************/
         ...
         $scope.initNavigation = function()
         {
             ...
             Navigation.navBarPush({name:"Προϊόντα",link:"products",current:true});
         }
