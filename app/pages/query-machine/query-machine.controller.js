(function () {
  ...
  angular
    .module("TIMECAPSULE")
    .controller("queryMachineController", queryMachineController);

  // Inject services to the Controller
  queryMachineController.$inject = [
    "$scope",
    "$state",
    "$window",
    "Global",
  ...
  // Controller Logic
  function queryMachineController(
    $scope,
    $state,
    $window,
    Global,
    ...
    $scope.$watch(
      function () {
        return $scope.query.markersReady;
      },
      function () {
        if (
          $scope.query.markersReady &&
          $scope.query.resultsTotal &&
          $scope.query.resultsTotal.length
        ) {
          $scope.initMap();
        }
      },
      ...
      for (var i = 0; i < $scope.query.resultsTotal.length; i++) {
        ...
        if ($scope.query.resultsTotal[i].type) {
          if ($scope.query.resultsTotal[i].type.value == "Grows")
            ...
            $scope.query.resultsTotal[i].type.value == "ArchaeologicalFinding"
          )
            markerIcon = "assets/images/marker-arcaeological.svg";
          else if ($scope.query.resultsTotal[i].type.value == "Cargo")
            markerIcon = "assets/images/marker-cargo.svg";
        }

        if (
          $scope.query.resultsTotal[i].latitude &&
          $scope.query.resultsTotal[i].longitude
        ) {
    ...
    $scope.addQueryToCompare = function () {
      if (!$scope.query.results.length) {
        ...
        $scope.popup.dialog = Util.showPopup(
          $scope.popupUrl + "error.message.popup.html",
          $scope
        );
      } else {
    ...
    $scope.isVisualization = function (vizName) {
      for (var i = 0; i < $scope.query.visualizations.length; i++) {
        if ($scope.query.visualizations[i] == vizName) return true;
      }
    ...
    $scope.queryAddFilterDatePeriodData = function () {
      ...
      if (index != -1) {
        $scope.query.currentFilter.data.name =
          $scope.data.datePeriod.min + " - " + $scope.data.datePeriod.max;
        $scope.query.currentFilter.data.dateFrom =
          $scope.data.datePeriod.min + "-01-01";
        $scope.query.currentFilter.data.dateTo =
          $scope.data.datePeriod.max + "-01-01";
        $scope.query.currentFilter.data.yearFrom = parseInt(
          $scope.data.datePeriod.min
        );
        $scope.query.currentFilter.data.yearTo = parseInt(
          $scope.data.datePeriod.max
        );
      } else {
    ...
    $scope.isFilterDataSelected = function (item) {
      return (
        $scope.query.currentFilter &&
        $scope.query.currentFilter.data &&
        $scope.query.currentFilter.data.name == item.name
      );
    };
    ...
    $scope.isFromDataSelected = function (item) {
      return (
        $scope.query.from &&
        $scope.query.from.data &&
        $scope.query.from.data.name == item.name
      );
    };
    ...
    $scope.showExportPopup = function (exportFormat) {
      $scope.message.clear();
      $scope.popup.dialog = Util.showPopup(
        $scope.popupUrl + "export.popup.html",
        $scope
      );
    };
    ...
    $scope.showCompareQueriesPopup = function () {
      $scope.message.clear();
      $scope.popup.dialog = Util.showPopup(
        $scope.popupUrl + "compare.queries.popup.html",
        $scope
      );
    };
    ...
    $scope.showComparePopup = function () {
      $scope.message.clear();
      $scope.popup.dialog = Util.showPopup(
        $scope.popupUrl + "compare.popup.html",
        $scope
      );
    };
    ...
    $scope.showCreateTimecapsulePopup = function () {
      ...
      $scope.message.clear();
      $scope.popup.dialog = Util.showPopup(
        $scope.popupUrl + "create.timecapsule.popup.html",
        $scope
      );
    };
    ...
    $scope.showQueryCriteriaPopup = function () {
      $scope.message.clear();
      $scope.popup.dialog = Util.showPopup(
        $scope.popupUrl + "query.criteria.popup.html",
        $scope
      );
    };
    ...
    $scope.showQueryFiltersPopup = function () {
      $scope.message.clear();
      $scope.popup.dialog = Util.showPopup(
        $scope.popupUrl + "query.filters.popup.html",
        $scope
      );
    };
    ...
    $scope.showQueryFromPopup = function () {
      ...
      $scope.popup.dialog = Util.showPopup(
        $scope.popupUrl + "query.from.popup.html",
        $scope
      );
    };
    ...
    $scope.showQuerySelectPopup = function () {
      ...
      $scope.popup.dialog = Util.showPopup(
        $scope.popupUrl + "query.select.popup.html",
        $scope
      );
    };
